...
| **Asset Type** | Document |
| **Control Record** | Document Maintenance Agreement |
...
| POST | `/v1/document-maintenance-agreement/initiate` | Initiate |
| GET | `/v1/document-maintenance-agreement` | Retrieve (list) |
| GET | `/v1/document-maintenance-agreement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/document-maintenance-agreement/{crId}/update` | Update |
| PUT | `/v1/document-maintenance-agreement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/document-maintenance-agreement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-document-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-document-management ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-document-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
